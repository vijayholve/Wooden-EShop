 ...
 */
const AuthForm = ({ type, setMessage }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isLogin) {
        // On successful login, call the context's login function and navigate to home
        const decoded = parseJwt(data.access);
        await login(
          data.access,
          data.refresh,
          decoded?.username || formData.username
        );
      ...
      // login is provided by AuthContext via useAuth() — no local definition here
    } catch (err) {
